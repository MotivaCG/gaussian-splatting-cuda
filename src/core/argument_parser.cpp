    ...
    namespace {
        ...
        constexpr std::array OPTIMIZATION_CLI_BINDINGS{
            ...
            OptimizationCliBinding{"--bg-mode", "bg_mode", Enum, false,
                                   "; values: solidcolor, modulation, image, random, pseudorandom", "solid_color", "solidcolor"}, // SMN: pseudorandom
            ...
            OptimizationCliBinding{"--mask-mode", "mask_mode", Enum, false,
                                   "; values: none, segment, ignore, segment_and_ignore, alpha_consistent, attention"}, // SMN: attention
...
namespace {
    ...
    std::optional<lfs::core::param::BackgroundMode> parse_bg_mode(const std::string& mode) {
        ...
        if (mode == "pseudorandom") // SMN
            return BackgroundMode::Pseudorandom;
        return std::nullopt;
    }
    ...
    std::expected<std::tuple<ParseResult, std::function<void()>>, std::string> parse_arguments(
        ...
        try {
            ...
            // SMN: "hybrid" (mcmc -> mrnf) isn't a registered strategy, so this flag keeps
            // its own literal help text instead of lfs::core::args::optimization_cli_help().
