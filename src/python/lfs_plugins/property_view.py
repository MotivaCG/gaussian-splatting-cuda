...
BASIC_RUNS = (
    # SMN begin — in-process cross-strategy switch: only shown while the "hybrid"
    # strategy preset (mcmc -> mrnf) is active.
    _run(
        "hybrid_at",
        "smn_switch_at_fraction",
        visibility_condition_id="dep_hybrid",
    ),
    # SMN end
...
_ENUM_OPTION_TOOLTIP_KEYS = {
    ...
    ("mask_mode", 5): "training.tooltip.opt.mask_mode.attention",  # SMN
    ...
    ("bg_mode", 4): "training.tooltip.opt.bg_mode.pseudorandom",  # SMN
}
