...
// SMN: post-training pruning passes ported from the previous method
// (gaussian-splatting-cuda src/training/smn/mask_pruning.*), plus a new
// Statistical Outlier Removal (SOR) pass. See src/SMN/README.md.
...
namespace lfs::training::smn {
    namespace mask_pruning {
        ...
        /**
         * @brief Configuration for Statistical Outlier Removal (SOR).
         *
         * Removal rule (per splat i, d_mean = mean distance to k nearest neighbors):
         *   d_mean(i) > mean(d_mean) + std_ratio * stddev(d_mean)
         *
         * Connectivity guard: protruding but LEGITIMATE geometry (a pointing
         * fingertip, a hair tip) has an elevated d_mean — all its neighbors sit
         * to one side — yet it is locally connected: its few nearest neighbors
         * are right next to it. A true straggler is far even from its guard_kth
         * neighbor. So removal additionally requires
         *   d_guard(i) > guard_multiplier * median(d_guard)
         * where d_guard = distance to the guard_kth nearest neighbor.
         */
        struct SORPruningConfig {
            /// Enable/disable this pass.
            bool enabled = true;

            /// Number of nearest neighbors averaged per splat (excluding self).
            /// Higher suits denser clouds; lower preserves fine detail. [1, 500]
            int k_neighbors = 20;

            /// Sensitivity multiplier over the global stddev. Lower = more
            /// aggressive (0.5-0.8), higher = conservative (2.0-3.0). [0.01, 10]
            float std_ratio = 1.5f;

            /// Connectivity guard: which neighbor is checked (<= 0 disables the
            /// guard). 4 pairs well with the isolation pass: clusters of up to
            /// 3 flyers still fail the guard, connected extremities pass it.
            int guard_kth = 4;

            /// A splat is only removable when its guard_kth-neighbor distance
            /// exceeds this multiple of the global median of that distance.
            float guard_multiplier = 3.0f;
        };
        ...
        /**
         * @brief Statistical Outlier Removal (SOR) — classic point-cloud denoise.
         *
         * For each splat, computes the MEAN distance to its k nearest neighbors
         * (excluding self). A splat is removed when its mean distance exceeds
         *
         *   global_mean + std_ratio * global_stddev
         *
         * of the mean-distance distribution over all splats. Unlike isolation
         * pruning (kth-neighbor vs. median multiplier, tuned for tiny flyer
         * clusters), SOR adapts to the global density statistics and catches
         * the sparse stragglers the other passes leave behind — run it LAST.
         *
         * Parameters follow the usual SOR presets:
         *   conservative k=30 ratio=2.0 | balanced k=20 ratio=1.5 | aggressive k=10 ratio=1.0
         */
